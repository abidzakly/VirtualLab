        ...
        Card(
            ...
            Row(
                ...
                OutlinedButton(
                    ...
                    border = BorderStroke(1.dp, DarkBlueDarker)
                ) {
                    RegularText(text = "Cancel", color = DarkBlueDarker)
                }
                Button(
                    onClick = { onConfirmation(title, type, price) },
                    enabled = title.isNotEmpty() && type.isNotEmpty() && price.isNotEmpty(),
                    modifier = Modifier.padding(8.dp),
                    colors = buttonColors(if (title.isNotEmpty() && type.isNotEmpty() && price.isNotEmpty()) DarkBlueDarker else GrayTextField)
                ) {
                    if (title.isNotEmpty() && type.isNotEmpty() && price.isNotEmpty())
                        RegularText(text = "Save", color = Color.White)
                    else {
                        RegularText(text = "Save", color = Color.DarkGray)
                    }
                }
            }
        }
